...
## Eleccion de la estrategia de join

La decision es del planner, que es quien tiene delante la condicion:
`HashJoin` si es una igualdad entre dos columnas, `NestedLoopJoin` en cualquier
otro caso. El executor solo instancia lo que el plan le pide. `EXPLAIN` muestra
cual se eligio.

## EXPLAIN

`EXPLAIN` no ejecuta la consulta. El planner construye el plan igual que
siempre, `planner.Explain` lo dibuja, y el motor devuelve ese texto como un
resultado normal de una columna usando el operador `Values`. Asi ni el CLI ni
el REPL necesitan un camino aparte para mostrarlo.
