...
func TestConsultaDistinct(t *testing.T) {
	motor := nuevoMotor(t, map[string]string{
		"ventas": "zona,monto\nNorte,10\nSur,20\nNorte,10\nNorte,30\n",
	})

	t.Run("elimina repetidas conservando el orden", func(t *testing.T) {
		rows := consultar(t, motor, "SELECT DISTINCT zona FROM ventas")
		if len(rows) != 2 {
			t.Fatalf("filas = %d; se esperaban 2", len(rows))
		}
		if rows[0][0].Data != "Norte" || rows[1][0].Data != "Sur" {
			t.Errorf("filas = %v, %v", rows[0][0].Data, rows[1][0].Data)
		}
	})

	t.Run("conserva el esquema de la proyeccion", func(t *testing.T) {
		got := columnas(t, motor, "SELECT DISTINCT zona, monto AS importe FROM ventas")
		if len(got) != 2 || got[0] != "zona" || got[1] != "importe" {
			t.Errorf("columnas = %v; se esperaban zona e importe", got)
		}
	})

	t.Run("compara la fila completa", func(t *testing.T) {
		rows := consultar(t, motor, "SELECT DISTINCT zona, monto FROM ventas")
		if len(rows) != 3 {
			t.Errorf("filas = %d; se esperaban 3 (Norte/10 esta repetida)", len(rows))
		}
	})

	t.Run("sin DISTINCT no elimina nada", func(t *testing.T) {
		if rows := consultar(t, motor, "SELECT zona FROM ventas"); len(rows) != 4 {
			t.Errorf("filas = %d; se esperaban 4", len(rows))
		}
	})

	t.Run("trata los NULL como un unico valor", func(t *testing.T) {
		// La segunda columna existe solo para que las celdas vacias de la
		// primera no queden en lineas en blanco, que el lector de CSV descarta.
		conNulos := nuevoMotor(t, map[string]string{"t": "v,w\n1,a\n,b\n2,c\n,d\n"})
		rows := consultar(t, conNulos, "SELECT DISTINCT v FROM t")
		if len(rows) != 3 {
			t.Fatalf("filas = %d; se esperaban 3 (1, NULL, 2)", len(rows))
		}
		if !rows[1][0].Null {
			t.Errorf("la segunda fila deberia ser NULL: %#v", rows[1][0])
		}
	})
}

func TestConsultaExplainNoEjecuta(t *testing.T) {
	motor := nuevoMotor(t, map[string]string{"empleados": empleadosCSV})

	got := columnas(t, motor, "EXPLAIN SELECT nombre FROM empleados")
	if len(got) != 1 || got[0] != "plan" {
		t.Fatalf("columnas = %v; se esperaba una sola columna llamada plan", got)
	}

	rows := consultar(t, motor, "EXPLAIN SELECT nombre FROM empleados WHERE edad >= 18")
	if len(rows) != 3 {
		t.Fatalf("filas = %d; se esperaban 3 lineas de plan", len(rows))
	}
	if rows[0][0].Data != "Project(nombre)" {
		t.Errorf("primera linea = %v", rows[0][0].Data)
	}
	if !strings.Contains(rows[2][0].Data.(string), "Scan(empleados)") {
		t.Errorf("ultima linea = %v", rows[2][0].Data)
	}
}

// TestConsultaAceptaPuntoYComa cubre el requisito de poder escribir la consulta
// con o sin punto y coma final.
func TestConsultaAceptaPuntoYComa(t *testing.T) {
	motor := nuevoMotor(t, map[string]string{"empleados": empleadosCSV})
	...
	for _, sql := range []string{
		...
		if rows := consultar(t, motor, sql); len(rows) != 3 {
			t.Errorf("%q devolvio %d filas; se esperaban 3", sql, len(rows))
		}
	}

	if _, err := motor.Query("SELECT nombre FROM empleados;;"); err == nil {
		t.Error("se admitieron dos puntos y coma")
	}
}

// TestErroresSemanticosCitanLaPosicion comprueba que los errores de las etapas
// posteriores al parser tambien dicen donde esta el problema.
func TestErroresSemanticosCitanLaPosicion(t *testing.T) {
	motor := nuevoMotor(t, map[string]string{"empleados": empleadosCSV})
	...
	tests := []struct {
		...
		{name: "columna en SELECT", sql: "SELECT sueldo FROM empleados"},
		{name: "columna en WHERE", sql: "SELECT nombre FROM empleados WHERE sueldo > 1"},
		{name: "funcion desconocida", sql: "SELECT stddev(edad) FROM empleados"},
		{name: "columna fuera de GROUP BY", sql: "SELECT nombre, COUNT(*) FROM empleados GROUP BY activo"},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !strings.Contains(err.Error(), "posicion") {
				t.Errorf("el error no cita la posicion: %v", err)
			}
		})
	}
}

// TestJoinSinIgualdadUsaNestedLoop comprueba que una condicion que el hash join
// no sabe resolver ya no falla.
func TestJoinSinIgualdadUsaNestedLoop(t *testing.T) {
	motor := nuevoMotor(t, map[string]string{
		"a": "x\n1\n2\n3\n",
		"b": "y\n1\n2\n",
	})

	rows := consultar(t, motor, "SELECT a.x, b.y FROM a INNER JOIN b ON a.x > b.y ORDER BY a.x, b.y")
	// 2>1, 3>1, 3>2
	if len(rows) != 3 {
		t.Fatalf("filas = %d; se esperaban 3", len(rows))
	}
	if rows[0][0].Data != int64(2) || rows[0][1].Data != int64(1) {
		t.Errorf("primera fila = %v, %v; se esperaba 2, 1", rows[0][0].Data, rows[0][1].Data)
	}
}

// TestJoinAmbasEstrategiasCoinciden compara los dos algoritmos a traves del
// motor completo, usando una condicion de igualdad con claves repetidas.
func TestJoinAmbasEstrategiasCoinciden(t *testing.T) {
	motor := nuevoMotor(t, map[string]string{
		"a": "k,v\n1,uno\n2,dos\n1,otro\n",
		"b": "k,w\n1,A\n1,B\n2,C\n3,D\n",
	})

	// La igualdad usa hash join; la condicion equivalente escrita al reves con
	// dos comparaciones fuerza el nested-loop.
	hash := consultar(t, motor, "SELECT a.v, b.w FROM a INNER JOIN b ON a.k = b.k ORDER BY a.v, b.w")
	nested := consultar(t, motor, "SELECT a.v, b.w FROM a INNER JOIN b ON a.k >= b.k AND a.k <= b.k ORDER BY a.v, b.w")

	if len(hash) != len(nested) {
		t.Fatalf("filas = %d (hash) y %d (nested-loop)", len(hash), len(nested))
	}
	if len(hash) != 5 {
		t.Fatalf("filas = %d; se esperaban 5 (uno:A, uno:B, otro:A, otro:B, dos:C)", len(hash))
	}
	for index := range hash {
		if hash[index][0].Data != nested[index][0].Data || hash[index][1].Data != nested[index][1].Data {
			t.Errorf("fila %d: hash=%v/%v nested=%v/%v", index,
				hash[index][0].Data, hash[index][1].Data,
				nested[index][0].Data, nested[index][1].Data)
		}
	}
}
