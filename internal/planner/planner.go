...
// joinStrategy elige el algoritmo del join. El hash join solo sabe resolver una
// igualdad entre dos columnas; cualquier otra condicion se resuelve comparando
// todos los pares con nested-loop.
func joinStrategy(condition parser.Expression) JoinStrategy {
	binary, ok := condition.(parser.BinaryExpression)
	if !ok || binary.Operator != parser.OpEqual {
		return NestedLoopStrategy
	}
	if _, ok := binary.Left.(parser.Identifier); !ok {
		return NestedLoopStrategy
	}
	if _, ok := binary.Right.(parser.Identifier); !ok {
		return NestedLoopStrategy
	}
