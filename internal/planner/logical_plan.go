...
// JoinStrategy indica con que algoritmo se resuelve un join.
type JoinStrategy uint8

const (
	// HashJoinStrategy indexa el lado derecho; solo sirve para igualdades
	// entre columnas.
	HashJoinStrategy JoinStrategy = iota
	// NestedLoopStrategy compara cada par de filas; admite cualquier condicion.
	NestedLoopStrategy
)

func (s JoinStrategy) String() string {
	if s == NestedLoopStrategy {
		return "NestedLoop"
	}
	return "Hash"
}

// Join combina dos entradas concatenando sus esquemas. La estrategia la elige
// el planner, que es quien tiene delante la condicion.
type Join struct {
	Left      Node
	Right     Node
...
// Distinct elimina las filas repetidas. Va por encima de Project, porque la
// repeticion se juzga sobre las columnas de salida.
