...
func TestPlanEligeLaEstrategiaDelJoin(t *testing.T) {
	tests := []struct {
		name string
		on   string
		want JoinStrategy
	}{
		{name: "igualdad entre columnas", on: "empleados.area_id = areas.id", want: HashJoinStrategy},
		{name: "mayor que", on: "empleados.area_id > areas.id", want: NestedLoopStrategy},
		{name: "distinto", on: "empleados.area_id <> areas.id", want: NestedLoopStrategy},
		{name: "igualdad contra un literal", on: "empleados.area_id = 1", want: NestedLoopStrategy},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			project := planificar(t, "SELECT empleados.nombre FROM empleados INNER JOIN areas ON "+test.on).(*Project)
			join, ok := project.Input.(*Join)
			if !ok {
				t.Fatalf("Project no lee de un Join: %T", project.Input)
			}
			if join.Strategy != test.want {
				t.Errorf("estrategia = %s; se esperaba %s", join.Strategy, test.want)
			}
		})
	}
}
...
func TestPlanDistinct(t *testing.T) {
	distinct, ok := planificar(t, "SELECT DISTINCT nombre FROM empleados").(*Distinct)
	if !ok {
		t.Fatalf("la raiz no es Distinct")
	}
	if _, ok := distinct.Input.(*Project); !ok {
		t.Fatalf("Distinct no lee de un Project: %T", distinct.Input)
	}
}

// TestExplainDibujaElArbol fija el formato exacto del plan, que es lo que ve el
// usuario con EXPLAIN.
func TestExplainDibujaElArbol(t *testing.T) {
	got := Explain(planificar(t, "SELECT nombre, salario FROM empleados WHERE salario >= 18"))
	want := "Project(nombre, salario)\n" +
		"└── Filter(salario >= 18)\n" +
		"    └── Scan(empleados)\n"
	if got != want {
		t.Errorf("plan =\n%s\nse esperaba\n%s", got, want)
...
// TestExplainDibujaLasDosRamasDeUnJoin comprueba el conector de rama intermedia.
func TestExplainDibujaLasDosRamasDeUnJoin(t *testing.T) {
	got := Explain(planificar(t, "SELECT empleados.nombre FROM empleados INNER JOIN areas ON empleados.area_id = areas.id"))
	want := "Project(empleados.nombre)\n" +
		"└── HashJoin(empleados.area_id = areas.id)\n" +
		"    ├── Scan(empleados)\n" +
		"    └── Scan(areas)\n"
	if got != want {
		t.Errorf("plan =\n%s\nse esperaba\n%s", got, want)
...
func TestExplainDescribeTodasLasClausulas(t *testing.T) {
	sql := "SELECT DISTINCT area_id, COUNT(*) FROM empleados WHERE salario > 0 " +
		"GROUP BY area_id HAVING COUNT(*) > 1 ORDER BY area_id DESC LIMIT 5 OFFSET 2"
	got := Explain(planificar(t, sql))

	for _, want := range []string{
		"Limit(max 5, offset 2)",
		"Distinct",
		"Project(area_id, COUNT(*))",
		"Sort(area_id DESC)",
		"Filter(COUNT(*) > 1)",
		"Aggregate(por area_id, COUNT(*))",
		"Filter(salario > 0)",
		"Scan(empleados)",
	} {
		if !strings.Contains(got, want) {
			t.Errorf("el plan no incluye %q:\n%s", want, got)
		}
	}
}
