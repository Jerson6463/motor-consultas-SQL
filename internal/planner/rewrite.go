...
func (r *rewriter) rewriteCall(call parser.FunctionCall) (parser.Expression, error) {
	...
	if !ok {
		return nil, fmt.Errorf("la funcion %q no existe (posicion %d)", call.Name, call.Position)
	}
	if call.Star && !spec.AcceptsStar {
		return nil, fmt.Errorf("%s requiere una columna (posicion %d)", name, call.Position)
	}
	if !call.Star && len(call.Args) != 1 {
		return nil, fmt.Errorf("%s requiere exactamente un argumento (posicion %d)", name, call.Position)
	}
...
func (r *rewriter) validate(expression parser.Expression) error {
	...
	case parser.Identifier:
		if !r.produced[storage.NormalizeName(expression.Name)] {
			return fmt.Errorf("la columna %q debe aparecer en GROUP BY (posicion %d)",
...
func checkFunctions(expression parser.Expression) error {
	...
	case parser.FunctionCall:
		if !functions.IsAggregate(expression.Name) {
			return fmt.Errorf("la funcion %q no existe (posicion %d)", expression.Name, expression.Position)
		}
