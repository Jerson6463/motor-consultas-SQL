...
func tablaAreas(t *testing.T) *storage.Table {
	t.Helper()
	table, err := storage.LoadCSV("areas", strings.NewReader("zona,jefe\nNorte,Ana\nSur,Beto\nNorte,Carla\n"))
	if err != nil {
		t.Fatalf("LoadCSV devolvio error: %v", err)
	}
	return table
...
// planDeJoin arma un join de ventas con areas usando la estrategia indicada.
func planDeJoin(t *testing.T, estrategia planner.JoinStrategy, condicion string) planner.Node {
	t.Helper()
	query, err := parser.Parse("SELECT * FROM t WHERE " + condicion)
	if err != nil {
		t.Fatalf("Parse devolvio error: %v", err)
	}
	return &planner.Join{
		Left:      &planner.Scan{Table: tablaVentas(t), Alias: "ventas"},
		Right:     &planner.Scan{Table: tablaAreas(t), Alias: "areas"},
		Condition: query.Where,
		Strategy:  estrategia,
	}
}

// TestJoinEquivalenciaEntreEstrategias comprueba que hash join y nested-loop
// producen exactamente las mismas filas para una condicion de igualdad.
func TestJoinEquivalenciaEntreEstrategias(t *testing.T) {
	condicion := "ventas.zona = areas.zona"

	hash, err := Build(planDeJoin(t, planner.HashJoinStrategy, condicion))
	if err != nil {
		t.Fatalf("Build (hash) devolvio error: %v", err)
	}
	defer hash.Close()

	nested, err := Build(planDeJoin(t, planner.NestedLoopStrategy, condicion))
	if err != nil {
		t.Fatalf("Build (nested-loop) devolvio error: %v", err)
	}
	defer nested.Close()

	if len(hash.Columns()) != len(nested.Columns()) {
		t.Fatalf("esquemas distintos: %d y %d columnas", len(hash.Columns()), len(nested.Columns()))
	}
	for index, column := range hash.Columns() {
		if nested.Columns()[index].Name != column.Name {
			t.Errorf("columna %d = %q y %q", index, column.Name, nested.Columns()[index].Name)
		}
	}

	filasHash := filas(t, hash)
	filasNested := filas(t, nested)
	if len(filasHash) != len(filasNested) {
		t.Fatalf("filas = %d (hash) y %d (nested-loop)", len(filasHash), len(filasNested))
	}
	if len(filasHash) == 0 {
		t.Fatal("el join no produjo filas; la prueba no comprueba nada")
	}
	for index := range filasHash {
		for columna := range filasHash[index] {
			if filasHash[index][columna].Data != filasNested[index][columna].Data {
				t.Errorf("fila %d columna %d: hash=%v nested=%v",
					index, columna, filasHash[index][columna].Data, filasNested[index][columna].Data)
			}
		}
	}
}

// TestNestedLoopResuelveCondicionesSinIgualdad cubre lo que el hash join no
// puede hacer y que antes fallaba.
func TestNestedLoopResuelveCondicionesSinIgualdad(t *testing.T) {
	plan := planDeJoin(t, planner.NestedLoopStrategy, "ventas.monto > 15")
	...
	// monto > 15 se cumple para 20 y 30, cruzados con las 3 areas.
	if rows := filas(t, operator); len(rows) != 6 {
		t.Errorf("filas = %d; se esperaban 6", len(rows))
	}
}

// contadorDeCierres envuelve un operador para contar sus Close().
type contadorDeCierres struct {
	Operator
	cierres int
}

func (c *contadorDeCierres) Close() error {
	c.cierres++
	return c.Operator.Close()
}

// TestJoinCierraAmbasEntradas comprueba que la entrada derecha, que se drena al
// construir el join, tambien se cierra.
func TestJoinCierraAmbasEntradas(t *testing.T) {
	condicion, err := parser.Parse("SELECT * FROM t WHERE ventas.zona = areas.zona")
	if err != nil {
		t.Fatalf("Parse devolvio error: %v", err)
	}
	...
	tests := []struct {
		name  string
		build func(left, right Operator) (Operator, error)
	}{
		{
			name: "hash",
			build: func(left, right Operator) (Operator, error) {
				return NewHashJoin(left, right, condicion.Where)
			},
		},
		{
			name: "nested-loop",
			build: func(left, right Operator) (Operator, error) {
				return NewNestedLoopJoin(left, right, condicion.Where)
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			left := &contadorDeCierres{Operator: NewScan(tablaVentas(t), "ventas")}
			right := &contadorDeCierres{Operator: NewScan(tablaAreas(t), "areas")}

			join, err := test.build(left, right)
			if err != nil {
				t.Fatalf("construir el join devolvio error: %v", err)
			}
			if err := join.Close(); err != nil {
				t.Fatalf("Close devolvio error: %v", err)
			}

			if left.cierres != 1 {
				t.Errorf("cierres del lado izquierdo = %d; se esperaba 1", left.cierres)
			}
			if right.cierres != 1 {
				t.Errorf("cierres del lado derecho = %d; se esperaba 1", right.cierres)
			}
		})
	}
}
