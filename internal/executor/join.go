...
func NewNestedLoopJoin(left, right Operator, condition parser.Expression) (*NestedLoopJoin, error) {
	...
	return &NestedLoopJoin{left: left, right: right, rightRows: rightRows, condition: condition, columns: columns}, nil
}
...
func (j *NestedLoopJoin) Close() error              { return closeBoth(j.left, j.right) }

// closeBoth cierra las dos entradas de un join y devuelve el primer error,
// para que un fallo al cerrar una no impida cerrar la otra.
func closeBoth(left, right Operator) error {
	err := left.Close()
	if rightErr := right.Close(); err == nil {
		err = rightErr
	}
	return err
}
