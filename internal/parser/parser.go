...
// ParseTokens analiza una lista de tokens terminada en EOF. Se admite un punto
// y coma final, opcional, para poder escribir las consultas como es habitual.
func ParseTokens(tokens []lexer.Token) (*Query, error) {
	...
	parser.match(lexer.SemicolonToken)
...
func (p *parser) parseQuery() (*Query, error) {
	query := &Query{}
	query.Explain = p.match(lexer.ExplainToken)

	if _, err := p.expect(lexer.SelectToken); err != nil {
		return nil, err
	}
	query.Distinct = p.match(lexer.DistinctToken)
...
func (p *parser) parsePrimary() (Expression, error) {
	...
	case lexer.IdentifierToken:
		...
		if p.current().Kind == lexer.LeftParenToken {
			return p.parseFunctionCall(token.Lexeme, token.Position)
		}
		return Identifier{Name: token.Lexeme, Position: token.Position}, nil
...
func (p *parser) parseFunctionCall(name string, position int) (Expression, error) {
	if _, err := p.expect(lexer.LeftParenToken); err != nil {
		return nil, err
	}

	call := FunctionCall{Name: name, Position: position}
