...
func Run(arguments []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	case "repl":
		return repl(arguments[1:], stdin, stdout, stderr)
	default:
		printUsage(stderr)
		return 1
	}
...
func queryMultiple(arguments []string, stdout, stderr io.Writer) int {
	...
	motor, ok := loadSources(arguments[:separator], stderr)
	if !ok {
		return 1
	}
...
// loadSources carga una lista de fuentes con la forma tabla=archivo.csv.
func loadSources(sources []string, stderr io.Writer) (*engine.Engine, bool) {
	motor := engine.New()
...
func printUsage(stderr io.Writer) {
	...
	fmt.Fprintln(stderr, "  sqlmem repl [tabla=archivo.csv ...]")
}
